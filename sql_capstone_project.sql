...
select ship_name from orders group by 1;

--hangi ürünler için stok güncellemesi yapılmalı?
select product_id,
product_name,
unit_in_stock,
reorder_level
from products
where reorder_level- unit_in_stock>0
and discontinued=0
...
--Çalışmaya devam edilen tedarikçiler ve sipariş sıklığı
select supplier_id,
       count (supplier_id) as Tedarikci_say,
	   count (product_id) as product_say
from products
where discontinued=0
group by 1 order by 1 desc;

--Tedariği ve satışı durdurulan ürünler
select p.product_name
from products as p
join order_details as od on od.product_id= p.product_id
join orders as o on o.order_id= od.order_id
where discontinued=1 and p.unit_in_stock=0 and unit_on_order=0 and p.reorder_level=0
group by 1 order by 1;

--satışı durdurulan ürünler(aynı şirketlerle çalışmaya devam ediliyor)
SELECT p.product_id,
p.product_name,
od.quantity,
od.discount,
...
o.required_date
FROM products AS p
right join suppliers as sup on sup.supplier_id=p.supplier_id
left JOIN order_details AS od ON od.product_id = p.product_id
JOIN orders AS o ON o.order_id = od.order_id
where p.unit_in_stock>0 and p.reorder_level>0 and discontinued=1
...
SELECT em.title,
	   ...
	   DATE_PART('year', AGE(max(o.order_date), em.hire_date)) AS calısma_suresi,
...
--en çok sipariş iptal eden şirketler
SELECT 
	o.ship_name,
    count (o.ship_via) as c_ship_via
FROM 
    orders as o
	JOIN customers AS cu ON cu.customer_id = o.customer_id
WHERE 
    (o.shipped_date - o.order_date) IS NULL
    OR (o.required_date - o.shipped_date) IS NULL
	group by 1
	order by 2 desc;
	
--sipariş iptalinin sebeplerinin incelenmesi
SELECT 
    o.ship_country,
    o.ship_via,
    p.product_name,
    cu.company_name,
    (o.shipped_date - o.order_date) AS kar_ver_s,
    (o.required_date - o.shipped_date) AS tesl_sur,
    (p.reorder_level - p.unit_in_stock) AS re_un_fark,
    o.order_date
FROM 
    orders AS o
...
WHERE 
	...
	AND p.reorder_level - p.unit_in_stock>=0
GROUP BY 
    o.ship_country, o.ship_via, p.product_name, cu.company_name, o.shipped_date, o.order_date, o.required_date, p.reorder_level, p.unit_in_stock
ORDER BY 
    re_un_fark desc;
	
--en çok satan 5 ürün
select p.product_name,
       count (o.order_id) as c_pr
from orders as o
    JOIN order_details AS od ON od.order_id = o.order_id
    JOIN products AS p ON p.product_id = od.product_id
	group by 1
...
from orders as o
	...
	order by 2 
	limit 5;
--en çok sipariş veren 10 şehir
select ship_city,
       count (order_id) as c_max_or
from orders
group by 1
order by 2 desc
limit 10

--indirim oranlarına göre sipariş sayısı

select p.product_name,
        od.discount,
       Count(od.order_id) as siparis_say
from products as p
       JOIN order_details AS od ON od.product_id = p.product_id
 where od.discount>0
	   group by 1,2
...
SELECT 
    p.product_name,
    od.discount,
    SUM(p.product_id) AS total_product_id,
    COUNT(*) AS discount_frequency  -- İndirim sıklığını hesaplar
FROM 
    products AS p
JOIN 
    order_details AS od 
...
WHERE 
    od.discount > 0
GROUP BY 
    p.product_name, od.discount
ORDER BY 
    p.product_name, od.discount DESC;

	
select p.product_name,
       count (od.order_id) as si_say,
	   od.discount,
	   o.order_date
from products as p
JOIN order_details AS od ON od.product_id = p.product_id
...
SELECT cu.company_name,
       (max (o.order_date)- min (o.order_date)) as sip_tar_fark
FROM customers AS cu
JOIN orders AS o ON cu.customer_id = o.customer_id
GROUP BY 1
ORDER BY sip_tar_fark
LIMIT 5;


--iletişim kuran ama sipariş vermeyen müşteri
SELECT cu.company_name, 
       o.order_id
FROM customers AS cu
...
--perakende alışveriş yapan müşteriler
SELECT cu.company_name
FROM customers AS cu
JOIN orders AS o ON cu.customer_id = o.customer_id
JOIN order_details AS od ON od.order_id = o.order_id
...
--toptan alışveriş yapan müşteriler
SELECT cu.company_name
FROM customers AS cu
JOIN orders AS o ON cu.customer_id = o.customer_id
JOIN order_details AS od ON od.order_id = o.order_id
...
--en fazla harcama yapan 5 müşteri
SELECT cu.company_name,
       SUM(od.quantity * od.unit_price * (1 - od.discount)) AS harcama
FROM customers AS cu
JOIN orders AS o ON cu.customer_id = o.customer_id
JOIN order_details AS od ON od.order_id = o.order_id
JOIN products AS p ON p.product_id = od.product_id
...
CREATE TABLE sadık_musteri_tablosu AS
SELECT cu.company_name,
       o.order_id,
       o.order_date
FROM customers as cu 
...
--Kargo fiyat kontrolü
select p.product_name, 
       od.unit_price * (1 - od.discount)as indirimli_fiyat,
       od.quantity,
	   o.freight, 
	   o.ship_name,
	   o.ship_via, 
...
--Çalışma süresi ve sipariş sayısısı arasındaki ilişki
SELECT em.title,
       em.employee_id, 
       CONCAT(em.first_name, ' ', em.last_name) AS full_name,
...
--91 farklı şirket
select count (distinct company_name) as comp_sayı
from customers;

--21 farklı ülke
select count (distinct ship_country) as ship_country_sayı
from orders;

--8 farklı kategori
select count (distinct category_name) as category_name_sayı
from categories;

--77 farklı ürün
select count (distinct product_name) as product_name_sayı
from products;
...
UPDATE shippers
SET phone = NULL
WHERE company_name IN ('Speedy Express', 'United Package', 'Federal Shipping');


UPDATE shippers
SET phone = CASE
    WHEN company_name = 'Speedy Express' THEN '0044-123-456'
    WHEN company_name = 'United Package' THEN '1-888-854-3899'
    WHEN company_name = 'Federal Shipping' THEN '1-800-247-4747'
    ELSE company_name  -- Diğer şirketler için mevcut telefon numarasını korur
END
WHERE company_name IN ('Speedy Express', 'United Package', 'Federal Shipping');
...
SELECT SUM(quantity*unit_price) as top_sip_tutarı
FROM order_details

WITH MaxPrices AS (
    SELECT 
        product_id,
        MAX(unit_price) AS max_fiyat
    FROM 
        order_details
    GROUP BY 
        product_id
...
SELECT 
    ROW_NUMBER() OVER (ORDER BY max_fiyat DESC) AS row_num,
    product_id,
    max_fiyat
FROM 
    MaxPrices
